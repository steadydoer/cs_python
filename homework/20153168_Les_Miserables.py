...
n = len(word_list) #총 단어 수



wordCountList = []#빈도수를 비교하기 위한 리스트 정의
for i in wordCount.items():#사전의 키와 값을 정렬 가능한 리스트로 만듦
...
print("%s is 1st word and percentage is %f%%." %(wordCountList[-1][0], (wordCountList[-1][1]/n)*100))
print("%s is 2nd word and percentage is %f%%." %(wordCountList[-2][0], (wordCountList[-2][1]/n)*100))
print("%s is 3rd word and percentage is %f%%." %(wordCountList[-3][0], (wordCountList[-3][1]/n)*100))
#for i in range(4, 31):
#    print("%s is %sth word and percentage is %f%%." %(wordCountList[i-1][0], i,(wordCountList[i-1][1]/n)*100))
